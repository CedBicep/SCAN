...
MakeSummaryOfThisMethod <- function(method,centrality,similarity,pvalue,distance)
{
	FileConn <- file("SummaryByMethod.txt","a")
	writeLines(paste(method,centrality,similarity,unlist(pvalue),unlist(distance),sep = "\t"), FileConn)
	close(FileConn)
}
...
Main <- function()
{
	...
	writeLines(paste("Method","Centrality","NetworkSimilarity","pvalue","Distance",sep="\t"), FileConn)
	close(FileConn)	
	
	FileConn <- file("CentralitySummary.txt","w")
	writeLines(paste(c("Similarity",attributes(data$metas)$names),collapse="\t"), FileConn)
	close(FileConn)
	
	FileConn <- file("NetworksSummary.txt","w")
	writeLines(paste(c("Method","Similarity","Network having the most good contigs","# of Good contigs"),collapse="\t"), FileConn)
	close(FileConn)
	...
	for(i in 1:length(data$metas))	#foreach method
	{
		...
		for(j in 1:length(data$metas[[i]]))	#for each similarity
		{
			...
			sif <- read.csv(as.character(data$metas[[i]][j]), head = FALSE, sep = "\t")
			...
			numOfContig <- sum(HasTag(graph,tag = tagtest))		#find total number of contig
			
			jaccard <- sapply(CC,HasGoodJaccard,tag1 = tagref,tag2 = tagtest)		#
			geodesics <- sapply(CC,GetlonguestGeod,tag1 = tagref,tag2 = tagtest)		#
			degree1 <- sapply(CC,GetPropDegree1,tag1 = tagref,tag2 = tagtest)		#
			artpointglobal <- sapply(CC,GetPropGlobalArtPoint,tag1 = tagref,tag2 = tagtest)	#
			artpointlocal <- sapply(CC,GetPropLocalArtPoint,tag1 = tagref,tag2 = tagtest)	#
			proportion <- sapply(CC,GetProportion,tag1 = tagref,tag2 = tagtest)		#
			...
			s <- MakeSummaryOfCentrality(fic = "ArticulationPoint_global_KS",tag1 = tagref,tag2 = tagtest,method = attributes(data$metas[i])$names,centrality = "ArticulationGlobal",similarity = names(data$metas[[i]][j]),pvalue = ks3$p.value,distance = ks3$statistic)
			s <- MakeSummaryOfCentrality(fic = "ArticulationPoint_local_KS",tag1 = tagref,tag2 = tagtest,method = attributes(data$metas[i])$names,centrality = "ArticulationLocal",similarity = names(data$metas[[i]][j]),pvalue = ks2$p.value,distance = ks2$statistic)
			s <- MakeSummaryOfCentrality(fic = "DegreeOne_KS",tag1 = tagref,tag2 = tagtest,method = attributes(data$metas[i])$names,centrality = "Degree1",similarity = names(data$metas[[i]][j]),pvalue = ks4$p.value,distance = ks4$statistic)
			s <- MakeSummaryOfCentrality(fic = "Longest_chain_KS",tag1 = tagref,tag2 = tagtest,method = attributes(data$metas[i])$names,centrality = "Geodesic",similarity = names(data$metas[[i]][j]),pvalue = ks5$p.value,distance = ks5$statistic)
			s <- MakeSummaryOfCentrality(fic = "Proportion_KS",tag1 = tagref,tag2 = tagtest,method = attributes(data$metas[i])$names,centrality = "Porportion",similarity = names(data$metas[[i]][j]),pvalue = ks1$p.value,distance = ks1$statistic)
			...
			s <- sapply(CC[which(Allresult[6,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_proportion",tag = tagtest)
			s <- sapply(CC[which(Allresult[6,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_proportion",tag = tagtest)
			s <- sapply(CC[which(Allresult[6,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_proportion",tag = tagtest)
			s <- sapply(CC[which(Allresult[6,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_proportion",tag = tagtest)
			
			s <- sapply(CC[which(Allresult[5,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_articulation_local",tag = tagtest)
			s <- sapply(CC[which(Allresult[5,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_articulation_local",tag = tagtest)
			s <- sapply(CC[which(Allresult[5,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_articulation_local",tag = tagtest)
			s <- sapply(CC[which(Allresult[5,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_articulation_local",tag = tagtest)
			
			s <- sapply(CC[which(Allresult[4,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_articulation_global",tag = tagtest)
			s <- sapply(CC[which(Allresult[4,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_articulation_global",tag = tagtest)
			s <- sapply(CC[which(Allresult[4,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_articulation_global",tag = tagtest)
			s <- sapply(CC[which(Allresult[4,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_articulation_global",tag = tagtest)
			
			s <- sapply(CC[which(Allresult[3,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_degree1",tag = tagtest)
			s <- sapply(CC[which(Allresult[3,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_degree1",tag = tagtest)
			s <- sapply(CC[which(Allresult[3,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_degree1",tag = tagtest)
			s <- sapply(CC[which(Allresult[3,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_degree1",tag = tagtest)
			
			s <- sapply(CC[which(Allresult[2,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_longest_chain",tag = tagtest)
			s <- sapply(CC[which(Allresult[2,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_longest_chain",tag = tagtest)
			s <- sapply(CC[which(Allresult[2,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_longest_chain",tag = tagtest)
			s <- sapply(CC[which(Allresult[2,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_longest_chain",tag = tagtest)
			
			s <- sapply(CC[which(Allresult[1,]==1)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SafeGeneFamilies_jaccard",tag = tagtest)
			s <- sapply(CC[which(Allresult[1,]==1)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SafeContigs_jaccard",tag = tagtest)
			s <- sapply(CC[which(Allresult[1,]==0)],WriteCC,sim = names(data$metas[[i]][j]),filename = "SuspiciousGeneFamilies_jaccard",tag = tagtest)
			s <- sapply(CC[which(Allresult[1,]==0)],WriteSeq,sim = names(data$metas[[i]][j]),filename = "SuspiciousContigs_jaccard",tag = tagtest)
		}
		...
		if(length(GoodSim) > 0)
		{
			if(length(GoodSim)==1)
			{
				...
				s <- MakeSummaryOfThisMethod(method = attributes(data$metas[i])$names,centrality = centralities,similarity = attributes(data$metas[[tagtest]])$names[GoodSim[1]],pvalue = AllPvalForThisMeth[[GoodSim[1]]],distance = AllDistForThisMeth[[GoodSim[1]]])
				
				FileConn <- file("NetworksSummary.txt","a")
				writeLines(paste(c( attributes(data$metas[i])$names, attributes(data$metas[[tagtest]])$names[GoodSim[1]], as.character(data$metas[[tagtest]][GoodSim[1]]),	numGoodContigForThisMeth[GoodSim[1]]),collapse="\t"), FileConn)
				close(FileConn)
			}
			else
			{
				...
				s <- MakeSummaryOfThisMethod(method = attributes(data$metas[i])$names,centrality = centralities,similarity = attributes(data$metas[[tagtest]])$names[totake],pvalue = AllPvalForThisMeth[[totake]],distance = AllDistForThisMeth[[totake]])
				
				FileConn <- file("NetworksSummary.txt","a")
				writeLines(paste(c( attributes(data$metas[i])$names, attributes(data$metas[[tagtest]])$names[totake], as.character(data$metas[[tagtest]][GoodSim[totake]]),	numGoodContigForThisMeth[GoodSim[totake]]),collapse="\t"), FileConn)
				close(FileConn)
			}
		}
	}
